...
- **Live progress** — a one-line call header (`spawn_agents (3 agents)`), then a labeled divider carrying the call-level summary (progress + elapsed while running; success count, duration, tool count, tokens and cost when done), then one block per agent: a quantified header line (status, duration, tools, tokens/cost — model only when the call mixes models) over an indented line showing the task while queued, the latest tool call while running, or the first line of the result once finished. `alt+a` opens the live detail panel; `Ctrl+O` after completion expands to each agent's final output.
