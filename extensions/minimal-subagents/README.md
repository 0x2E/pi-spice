...
- **Live progress** — a one-line call header (`spawn_agents (3 agents)`), then one indented block per agent — a quantified header line (status, duration, tools, tokens/cost — model only when the call mixes models) over the task while queued, the latest tool call while running, or the first line of the result once finished — closed by a footer line carrying the call-level summary and the hints (`⎿ 3/3 · 52s · 15 tools · ↓8.1k · $0.04 · alt+a timeline · ctrl+o expand`). `alt+a` opens the live detail panel; `Ctrl+O` after completion expands to each agent's final output.
